...
create table if not exists public.teamkracht_doel (
  ...
  -- 'sturen_doen_boven_norm' is de vaste regel van de opdrachtgever
  -- (07-09-2026): bij het doelbeeld en het eindbeeld moeten Sturen en Doen
  -- boven het landelijke beeld uitkomen. De schuifjes in het dashboard
  -- begrenzen daarop. De andere codes blijven beschikbaar.
  doel_code    text check (doel_code in ('sturen_doen_boven_norm','naar_landelijk','halverwege','bundel')),
...
values
('R1','Ziet alles, rent de andere kant op','{n_HLL} Zieners en {n_LHH} Aanpakkers','remt',
 '{"min": {"HLL": 2, "LHH": 1}}',
...
('R2','Eén stuurt, de rest wacht','Eén Trekker, {n_LLH} Uitvoerders en {n_LLL} Afwachters','remt',
...
('R3','Het levert, en niemand groeit',null,'remt',
 '{"min": {"HHL": 1, "LLH": 2}}',
...
('R4a','Veel richting, weinig beweging',null,'remt',
 '{"min": {"HHL": 2}, "max_n": 10}',
...
('R4b','Veel richting, weinig beweging',null,'remt',
 '{"min": {"HHL": 3}, "min_n": 11}',
 $t$Veel richting, weinig beweging. Plannen concurreren, besluiten worden overgedaan, niemand zet de eerste stap.$t$,
 $t$Dezelfde discussie in drie overleggen; lange overleggen, korte actielijsten.$t$,
 $t$Beslisdomeinen afspreken, en per besluit een eerste stap die de Beslisser zelf zet binnen een week.$t$,
 $t$Welk besluit is dit jaar meer dan één keer genomen, en wat is er sindsdien gedaan?$t$, 0.15, 5),
('R5','Iemand vangt het op, niemand ziet het',null,'remt',
...
('R6','Iedereen rent, niemand kijkt',null,'remt',
 '{"meerderheid": ["LHH"]}',
...
('R7','Het is stil, en dat is het signaal',null,'remt',
...
('R8','Signaal en besluit zitten aan één tafel','{n_HLL} Zieners naast {n_HHL} Beslissers','versterkt',
 '{"min": {"HLL": 1, "HHL": 1}}',
...
('R9','Wat tussen de rollen valt, blijft liggen',null,'remt',
 '{"min": {"LHL": 2}}',
...
('R10','Teamgeluk uit gemak','{n_MMM} van de {n} in de middenband','neutraal',
